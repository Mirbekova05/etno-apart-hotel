...
BOT_TOKEN = os.environ.get("BOT_TOKEN", "8953721099:AAEtwG8HqbT6DJvKzrxzvi_qhXBupXqcrZw")
...
def cancel_booking(row, booking, kept_amount=0):
    ...
    ws.update_cell(row, 3, f"ОТМЕНЕНО - {original_guest}")
    ws.update_cell(row, 9, kept_amount)
    ws.update_cell(row, 10, kept_amount)
    ws.update_cell(row, 12, 0)
    ws.update_cell(row, 14, 0)
...
def add_payment(row, booking, amount, method):
    ...
    if status == STATUS_BOOKED and current_prepay == 0:
        ws.update_cell(row, 10, amount)
        ws.update_cell(row, 11, method)
    else:
